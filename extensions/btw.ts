		...
		try {
			messages.push(
				...buildSessionContext(
					...
					(message) =>
						!isVisibleBtwMessage(
							message as { role: string; customType?: string },
						),
				),
			);
		} catch {
...
type EntryWithoutId = BtwTranscriptEntry extends infer E
	? E extends { id: number }
		? Omit<E, "id">
		: never
	: never;
...
function extractMessageText(message: { content?: unknown }): string {
	return Array.isArray(message.content)
		? extractText(message.content as AssistantMessage["content"], "text")
		: "";
}
...
function extractBtwHandoffThread(
	...
	const threadMessages = isThreadContinuationMarker(handoffMessages, 0)
...
class BtwOverlayComponent extends Container implements Focusable {
	...
	override render(width: number): string[] {
		...
		lines.push(
			this.frameLine(
				this.theme.fg("accent", this.theme.bold(this.modeText.text.trim())),
				innerWidth,
			),
		);
		...
		lines.push(
			this.frameLine(
				this.theme.fg("warning", this.statusText.text.trim()),
				innerWidth,
			),
		);
		...
		lines.push(
			this.frameLine(
				this.theme.fg("dim", this.hintsText.text.trim()),
				innerWidth,
			),
		);
...
export default function (pi: ExtensionAPI) {
	...
	function parseOverlayBtwCommand(
		...
		const match = trimmed.match(
			/^\/(btw:(?:new|tangent|clear|inject|summarize))(?:\s+(.*))?$/,
		);
		if (!match) {
			return null;
		}
