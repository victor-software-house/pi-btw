...
vi.mock("@earendil-works/pi-tui", async () => {
	const actual = await vi.importActual<typeof import("@earendil-works/pi-tui")>(
...
async function* streamAnswer(answer: string) {
	yield {
		...
		delta: answer.slice(0, Math.max(1, Math.floor(answer.length / 2))),
	};
	yield {
		type: "text_delta" as const,
		delta: answer.slice(Math.max(1, Math.floor(answer.length / 2))),
	};
	...
	return {
		...
		stream: async function* () {
			...
			yield {
				...
				result: {
					content: [
						{ type: "text", text: '{"name":"@victor-software-house/pi-btw"}' },
					],
				},
			};
...
function createBlockingAnswerStream(answer: string) {
	...
	return {
		...
		stream: async function* () {
			yield {
				type: "text_delta" as const,
				delta: answer.slice(0, firstChunkLength),
			};
			await blocked;
			yield {
				type: "text_delta" as const,
				delta: answer.slice(firstChunkLength),
			};
			yield {
				type: "done" as const,
	...
	const session = {
		...
		prompt: vi.fn(async (text: string) => {
			...
			const context: StreamContext = {
				...
				messages: [
					...stateMessages.map((message) => structuredClone(message)),
					userMessage,
				],
			};
			...
			const stream = promptStreamMock(
				record,
				text,
				context,
			) as AsyncIterable<PromptStreamEvent>;
			...
			const emitAssistantUpdate = (
				assistantMessageEvent: PromptStreamEvent,
			) => {
...
function createHarness(
	...
	const api: ExtensionAPI = {
		...
		sendMessage: ((message: unknown, options?: unknown) =>
			sentMessages.push({ message, options })) as any,
		sendUserMessage: ((content: unknown, options?: unknown) =>
			sentUserMessages.push({ content, options })) as any,
		appendEntry: ((customType: string, data?: unknown) =>
			entries.push({ type: "custom", customType, data })) as any,
	...
	const baseCtx = {
		...
		modelRegistry: {
			getApiKeyAndHeaders: vi.fn(async () =>
				hasCredentials
					? { ok: true, apiKey: "test-key", headers: {} }
	...
	async function runEvent(
		name: string,
		event: unknown = {},
		ctx: ExtensionContext | ExtensionCommandContext = baseCtx as any,
	) {
	...
	function latestWidgetFactory() {
		const widget = [...widgets]
			.reverse()
			.find(
				(entry) => entry.key === "btw" && typeof entry.content === "function",
			);
...
describe("btw runtime behavior", () => {
	beforeEach(() => {
		...
		createAgentSessionMock.mockImplementation(async (options: any) =>
			createMockAgentSession(options),
		);
		promptStreamMock.mockImplementation(
			(_record: unknown, _text: string, context: StreamContext) => {
				return streamAnswer(
					`default:${(context.messages.at(-1)?.content[0] as any)?.text ?? ""}`,
				);
			},
		);
	});
	...
	it("creates a BTW sub-session with an in-memory session manager, coding tools, and BTW system prompt", async () => {
		...
		expect(subSession.prompt).toHaveBeenCalledWith("first question", {
			source: "extension",
		});
	});
	...
	it("contextual BTW seeds the sub-session with main-session messages but excludes visible BTW notes", async () => {
		...
		const seedTexts = subSessionRecords[0].seedMessages.map(
			(message) => (message.content[0] as any)?.text ?? "",
		);
	...
	it("switching to tangent recreates the sub-session without inherited main-session context", async () => {
		...
		expect(
			contextualRecord.seedMessages.map(
				(message) => (message.content[0] as any)?.text ?? "",
			),
		).toContain("main session task");
		...
		expect(
			tangentRecord.seedMessages.map(
				(message) => (message.content[0] as any)?.text ?? "",
			),
	...
	it("preserves BTW overlay recoverability after agent prompt failure", async () => {
		...
		expect(getCustomEntries(harness.entries, "btw-thread-entry")).toHaveLength(
			0,
		);
	...
	it("allows main-session input to proceed while the BTW sub-session is streaming", async () => {
		...
		expect(
			findLatest(
				transcriptEntries(overlay),
				(entry: any) => entry.type === "tool-call",
			),
		).toMatchObject({
			toolName: "read",
			args: "package.json",
	...
	it("ignores late session events after overlay dismissal disposes the sub-session", async () => {
		...
		expect(transcriptText(reopened)).toContain(
			"No BTW thread yet. Ask a side question to start one.",
		);
	});
	...
	it("maps turn, tool, thinking, and assistant events into transcript entries", async () => {
		...
		promptStreamMock.mockImplementation(async function* () {
			...
			yield {
				type: "text_delta" as const,
				delta: "The package is @victor-software-house/pi-btw.",
			};
			yield {
				type: "done" as const,
				message: {
					...makeAssistantMessage(
						"The package is @victor-software-house/pi-btw.",
					),
		...
		expect(entries[5]).toMatchObject({
			type: "assistant-text",
			text: "The package is @victor-software-house/pi-btw.",
			streaming: false,
		});
	...
	it("renders tool, thinking, result, and turn-separator rows in the overlay transcript", async () => {
		...
		expect(transcript).toContain("Second answer");
		expect(transcript.indexOf("↳ result")).toBeGreaterThan(
			transcript.indexOf("<bold>read</bold>"),
		);
		expect(transcript.indexOf("second question")).toBeGreaterThan(
			transcript.indexOf("────────────────"),
		);
	});
	...
	it("transcript inspection exposes streaming and failure state", async () => {
		...
		expect(
			findLatest(entries, (entry: any) => entry.type === "thinking"),
		).toMatchObject({
			text: "Inspecting package.json",
			streaming: true,
		});
		expect(
			findLatest(entries, (entry: any) => entry.type === "tool-call"),
		).toMatchObject({
			toolName: "read",
			args: "package.json",
		});
		expect(entries.some((entry: any) => entry.type === "tool-result")).toBe(
			false,
		);
		...
		expect(
			findLatest(entries, (entry: any) => entry.type === "tool-result"),
		).toMatchObject({
			toolName: "read",
			content: '{"name":"@victor-software-house/pi-btw"}',
			truncated: false,
		...
		expect(
			findLatest(entries, (entry: any) => entry.type === "assistant-text"),
		).toMatchObject({
	...
	it("updates assistant transcript text incrementally while the BTW response streams", async () => {
		...
		expect(
			findLatest(
				transcriptEntries(overlay),
				(entry: any) => entry.type === "assistant-text",
			),
		).toMatchObject({
			text: "Partial",
			streaming: true,
	...
	it("surfaces missing credentials as an explicit error without creating a thread entry", async () => {
		...
		overlay.refresh();
		expect(overlay.statusText.text).toContain(
			"No credentials available for test-provider/test-model.",
		);
		expect(harness.notifications.at(-1)).toEqual({
	...
	it("renders BTW as a bordered dialog with an internal transcript viewport", async () => {
		...
		const longAnswer = Array.from(
			{ length: 24 },
			(_, index) => `line ${index + 1} of a long answer`,
		).join("\n");
	...
	it("keeps the BTW modal at a fixed reading height, uses one frame color, and preserves stacked body indentation", async () => {
		...
		const emptyStateLine = emptyLines.find((line: string) =>
			line.includes("No BTW thread yet."),
		);
		const inputLine = populatedLines.at(-3);
		const assistantBodyLine = populatedLines.find((line: string) =>
			line.includes("First answer"),
		);
		...
		expect(emptyStateLine).toContain(
			"<fg:borderMuted>│</fg:borderMuted><fg:dim>No BTW thread yet.",
		);
		expect(emptyStateLine).not.toContain(
			"<fg:borderMuted>│</fg:borderMuted> <fg:dim>No BTW thread yet.",
		);
		expect(assistantBodyLine).toContain(
			"<fg:borderMuted>│</fg:borderMuted>    First answer",
		);
	...
	it("/btw:new appends a reset marker, disposes the old sub-session, clears prior hidden thread state, stays contextual, and reopens a fresh thread", async () => {
		...
		promptStreamMock
			...
			.mockImplementationOnce(
				(_record: unknown, _text: string, context: StreamContext) => {
					const texts = context.messages.map(
						(message) => (message.content[0] as any)?.text ?? "",
					);
	...
	it("switching between /btw:tangent and /btw appends reset markers and tangent requests omit inherited main-session conversation", async () => {
		...
		const resets = getCustomEntries(harness.entries, "btw-thread-reset");
		expect(resets).toHaveLength(2);
		expect(resets.map((entry) => (entry.data as any)?.mode)).toEqual([
			"tangent",
			"contextual",
		]);

		const streamCalls = promptStreamMock.mock.calls as Array<
			[unknown, string, StreamContext]
		>;
		expect(streamCalls.length).toBeGreaterThanOrEqual(2);

		const callTexts = streamCalls.map((call) =>
			call[2].messages.map(
				(message) => (message.content[0] as any)?.text ?? "",
			),
		);
		const tangentTexts = callTexts.find(
			(texts) => texts.at(-1) === "tangent start",
		);
		...
		const contextualTexts = callTexts.find(
			(texts) => texts.at(-1) === "contextual start",
		);
		if (contextualTexts) {
	...
	it("/btw:clear dismisses the overlay, disposes the active sub-session, appends a reset marker, and restore only rehydrates entries after the last reset", async () => {
		...
		const resetCountBeforeClear = getCustomEntries(
			harness.entries,
			"btw-thread-reset",
		).length;
		...
		expect(transcriptText(overlay)).toContain(
			"No BTW thread yet. Ask a side question to start one.",
		);
	...
	it("/btw:clear during active tool execution aborts the prompt, disposes the sub-session, and leaves no partial thread", async () => {
		...
		expect(getCustomEntries(harness.entries, "btw-thread-reset")).toHaveLength(
			1,
		);
		expect(harness.notifications.at(-1)).toEqual({
			message: "Cleared BTW thread.",
		...
		await harness.command("btw", "");
		expect(transcriptText(harness.latestOverlayComponent())).toContain(
			"No BTW thread yet. Ask a side question to start one.",
		);
	});
	...
	it("/btw:inject while the main session is busy delivers to the main session as a follow-up", async () => {
		...
		expect(harness.sentUserMessages[0]).toEqual({
			content:
				"Here is a side conversation I had. Queue this behind the active turn.\n\nUser: busy question\nAssistant: Busy answer",
	...
	it("/btw:summarize success summarizes the active sub-session thread, disposes it, dismisses the overlay, and reopens fresh", async () => {
		...
		expect(harness.sentUserMessages[0]).toEqual({
			content:
				"Here is a summary of a side conversation I had. Hand this to the main agent.\n\nShort summary",
			options: undefined,
		});
		expect(getCustomEntries(harness.entries, "btw-thread-reset")).toHaveLength(
			1,
	...
	it("in-modal /btw:tangent reuses command semantics by switching modes and dropping inherited main-session context", async () => {
		...
		const tangentCall = [...streamCalls].reverse().find((call) => {
			const texts = call[2].messages.map(
				(message) => (message.content[0] as any)?.text ?? "",
			);
			return texts.at(-1) === "tangent start";
		});
		expect(tangentCall).toBeDefined();
		const tangentTexts = tangentCall![2].messages.map(
			(message) => (message.content[0] as any)?.text ?? "",
		);
		expect(tangentTexts).not.toContain("main session task");
	...
	it("in-modal /btw:inject reuses command semantics by handing off to the main session and dismissing the overlay", async () => {
		...
		expect(harness.sentUserMessages[0]).toEqual({
			content:
				"Here is a side conversation I had. Use this in the main run.\n\nUser: first question\nAssistant: First answer",
			options: undefined,
		});
		expect(getCustomEntries(harness.entries, "btw-thread-reset")).toHaveLength(
			1,
	...
	it("routes non-BTW slash input in the modal through the BTW sub-session prompt without fallback warnings", async () => {
		...
		const resetCountBefore = getCustomEntries(
			harness.entries,
			"btw-thread-reset",
		).length;
		...
		expect(record.session.prompt).toHaveBeenLastCalledWith(
			"/plan do something else",
			{ source: "extension" },
		);
		expect(record.promptCalls.at(-1)?.text).toBe("/plan do something else");
		expect(
			(record.promptCalls.at(-1)?.context.messages.at(-1)?.content[0] as any)
				?.text ?? "",
		).toBe("/plan do something else");
		...
		expect(
			harness.notifications.some((entry) =>
				entry.message.includes("Unsupported slash input in BTW"),
			),
		).toBe(false);
		expect(overlay.statusText.text).toContain("Ready for a follow-up");
